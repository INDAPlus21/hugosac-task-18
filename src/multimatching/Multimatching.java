...
class Multimatching {
	// Max number of states. The same as the max sum of 
	// the length of all keywords
	static int MAXS = 100000;

	// Maximum number of characters
	// in input alphabet (a-z, space, A-Z)
	static int MAXC = 27 + 26;
	...
	static int buildMatchingMachine(String arr[], int k) {
		...
		for (int i = 0; i < k; ++i) {
			...
			for (int j = 0; j < word.length(); ++j) {
				...
				if (word.charAt(j) == ' ') {
					ch = 26;
				} else if (Character.toLowerCase(word.charAt(j)) == word.charAt(j)) {
					ch = word.charAt(j) - 'a';
				} else {
					ch = word.charAt(j) - 'A' + 27;
				}
	...
	static int findNextState(int currentState, char nextInput) {
		...
		if (nextInput == ' ') {
			ch = 26;
		} else if (Character.toLowerCase(nextInput) == nextInput) {
			ch = nextInput - 'a';
		} else {
			ch = nextInput - 'A' + 27;
		}
	...
	static HashMap<String, ArrayList<Integer>> searchWords(String arr[], int k, String text) {
		// Preprocess patterns.
		...
		HashMap<String, ArrayList<Integer>> resultsMap = new HashMap<>(); 
		for (String s : arr) {
		...
		for(int i = 0; i < text.length(); ++i) {
			...
			for(int j = 0; j < k; ++j) {
				if ((out[currentState] & (1 << j)) > 0) {
					String word = arr[j];
					Integer index = i - word.length() + 1;
					resultsMap.get(word).add(index);
				}
			}
		}
		return resultsMap;
	...
	public static void main(String[] args) {
		...
		while (sc.hasNextLine()) {
			...
			if (lineIter == 0) {
				...
				HashMap<String, ArrayList<Integer>> results = searchWords(arr, k, line);
				
				// Print results
				for (String s : arr) {
					System.out.println(
						results.get(s).stream()
									.map(Object::toString)
